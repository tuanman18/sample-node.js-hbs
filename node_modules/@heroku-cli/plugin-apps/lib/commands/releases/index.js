...
function* run(context, heroku) {
    ...
    for (let row of releases) {
      for (let colKey in row) {
        ...
        for (let col of columns) {
          ...
          col.optimizationWidth = Math.max(col.optimizationWidth, formattedValue.length);
        }
      }
    }
  ...
  if (context.flags.json) {
    ...
    let options = {
      printHeader: false,
      columns: [{ key: 'version', format: (v, r) => cli.color[statusHelper.color(r.status)]('v' + v) }, { key: 'description', format: descriptionWithStatus }, { key: 'user', format: u => cli.color.magenta(u.email.replace(/@addons\.heroku\.com$/, '')) }, { key: 'created_at', format: t => time.ago(new Date(t)) }, { key: 'extended.slug_id' }, { key: 'extended.slug_uuid' }]
    };
    ...
    let options = {
      printHeader: false,
      columns: [{ key: 'version', label: '', format: (v, r) => cli.color[statusHelper.color(r.status)]('v' + v) }, { key: 'description', format: descriptionWithStatus }, { key: 'user', format: u => cli.color.magenta(u.email) }, { key: 'created_at', format: t => time.ago(new Date(t)) }]
    };
    optimizeWidth(releases, options.columns, 'description');
    cli.table(releases, options);
  }
...
Example:
  ...
  needsApp: true,
  needsAuth: true,
  flags: [{ name: 'num', char: 'n', description: 'number of releases to show', hasValue: true }, { name: 'json', description: 'output releases in json format' }, { name: 'extended', char: 'x', hidden: true }],
