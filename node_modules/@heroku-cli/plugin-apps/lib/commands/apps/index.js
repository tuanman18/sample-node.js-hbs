...
function* run(context, heroku) {
  ...
  function print(apps, user) {
    if (apps.length === 0) {
      if (space) cli.log(`There are no apps in space ${cli.color.green(space)}.`);else if (org) cli.log(`There are no apps in team ${cli.color.magenta(org)}.`);else cli.log('You have no apps.');
    } else if (space) {
      ...
      if (apps[1].length > 0) {
        ...
        cli.table(apps[1], {
          printHeader: false,
          columns: [{ key: 'name', get: regionizeAppName }, { key: 'owner.email' }]
        });
      }
    }
  }
  ...
  let path = '/users/~/apps';
  if (org) path = `/organizations/${org}/apps`;else if (context.flags.all) path = '/apps';
  let [apps, user] = yield [heroku.get(path), heroku.get('/account')];
...
module.exports = [Object.assign({ topic: 'apps' }, cmd), Object.assign({ topic: 'list', hidden: true }, cmd), Object.assign({ topic: 'apps', command: 'list', hidden: true }, cmd)];