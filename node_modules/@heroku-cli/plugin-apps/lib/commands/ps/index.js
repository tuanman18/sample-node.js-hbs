...
function printExtended(dynos) {
  ...
  cli.table(dynos, {
    columns: [{ key: 'id', label: 'ID' }, { key: 'name', label: 'Process' }, { key: 'state', label: 'State', format: (state, row) => `${state} ${time.ago(new Date(row.updated_at))}` }, { key: 'extended.region', label: 'Region' }, { key: 'extended.instance', label: 'Instance' }, { key: 'extended.ip', label: 'IP' }, { key: 'extended.port', label: 'Port' }, { key: 'extended.az', label: 'AZ' }, { key: 'release.version', label: 'Release' }, { key: 'command', label: 'Command', format: trunc }, { key: 'extended.route', label: 'Route' }, { key: 'size', label: 'Size' }]
  });
}
...
function* run(context, heroku) {
  ...
  if (json) cli.styledJSON(dynos);else if (extended) printExtended(dynos);else {
    yield printAccountQuota(context, heroku, appInfo, accountInfo);
    if (dynos.length === 0) cli.log(`No dynos on ${cli.color.app(app)}`);else printDynos(dynos);
  }
}
...
module.exports = {
  ...
  variableArgs: true,
  usage: 'ps [TYPE [TYPE ...]]',
  flags: [{ name: 'json', description: 'display as json' }, { name: 'extended', char: 'x', hidden: true }],
  help: `
Examples:
